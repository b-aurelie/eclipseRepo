...
public class PersonneDaoImpl implements Dao<Personne> {
	...
	public void save(Personne personne) {
		...
		if (c !=null) {
			try {
				PreparedStatement ps = c.prepareStatement("INSERT INTO Personne (nom,prenom,age) VALUES (?,?,?);");
	...
	public Personne findByNomPrenom(String nom, String prenom) {
		...
		if (c !=null) {
			try {
				...
				ResultSet r =ps.executeQuery();
				if (r.next())
					p = new Personne(r.getInt("numP"),r.getString("nom"),r.getString("prenom"),r.getString("age"));
				return p;
			} 
			catch (SQLException e) {
				e.printStackTrace();
